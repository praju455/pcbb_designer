"""DFM validation with Gemini analysis and fix suggestions."""
...
def _segments(text: str) -> list[tuple[float, float, float, float, float]]:
    """Extract PCB segments and widths."""

    pattern = re.compile(r'\(segment \(start ([\d\.-]+) ([\d\.-]+)\) \(end ([\d\.-]+) ([\d\.-]+)\) \(width ([\d\.-]+)\)')
...
def _distance(a: tuple[float, float, float, float, float], b: tuple[float, float, float, float, float]) -> float:
...
def _render(report: DFMReport, console: Console) -> None:
    ...
    for check in report.checks:
        icon = "[green]✅[/green]" if check.passed else "[red]❌[/red]" if check.severity == "error" else "[yellow]⚠️[/yellow]"
...
def validate_pcb(pcb_file: str | Path, fab_target: FabTarget = "generic", console: Console | None = None) -> DFMReport:
    ...
    min_trace = min((segment[4] for segment in segments), default=0.25)
    min_clearance = min((_distance(segments[index], segments[other]) for index in range(len(segments)) for other in range(index + 1, len(segments))), default=1.0)
    min_via = min((size for size, _ in vias), default=0.6)
    min_drill = min((drill for _, drill in vias), default=0.3)
    min_edge = min((min(x, y, width - x, height - y) for _, x, y, _ in footprints), default=10.0)
    ...
    checks = [
        ...
        _check("Min clearance", min_clearance >= settings.dfm_min_clearance_mm, "error", "Increase spacing between tracks.", f"{min_clearance:.3f} mm", f">= {settings.dfm_min_clearance_mm:.3f} mm"),
        _check("Min via diameter", min_via >= settings.dfm_min_via_diameter_mm, "error", "Use larger vias.", f"{min_via:.3f} mm", f">= {settings.dfm_min_via_diameter_mm:.3f} mm"),
        ...
        _check("Missing courtyard", not missing_courtyard, "warning", "Add F.CrtYd outlines.", "missing" if missing_courtyard else "present", "present"),
        _check("JLCPCB trace width", min_trace >= settings.jlcpcb_min_trace_width_mm, "info", "Keep traces above JLCPCB minimum.", f"{min_trace:.3f} mm", f">= {settings.jlcpcb_min_trace_width_mm:.3f} mm"),
        _check("Drill sizes", min_drill >= 0.2, "warning", "Increase drill size or confirm fab capability.", f"{min_drill:.3f} mm", ">= 0.200 mm"),
        _check("Copper pour connections", "segment" in text, "info", "Review pours in KiCad.", "present" if "segment" in text else "unknown", "present"),
        _check("Component overlap", not overlap, "error", "Spread footprints farther apart.", "overlap" if overlap else "clear", "clear"),
    ]

    score = max(0.0, 100.0 - sum(15 if not check.passed and check.severity == "error" else 6 if not check.passed else 0 for check in checks))
    summary = ""
    ...
    try:
        summary = verifier.generate(
            "Analyze this DFM report, suggest concrete fixes, and estimate fabrication success probability.\n\n"
