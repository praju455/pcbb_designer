  ...
  return (
    <form onSubmit={(event) => { event.preventDefault(); onSubmit({ description, optimize, skipVerify }); }} className="glass rounded-[2rem] p-6 md:p-8">
      ...
      <textarea
        ...
        className="field-shell min-h-44 w-full rounded-[1.5rem] p-5 outline-none transition focus:border-primary"
      />
      <div className="mt-5 grid gap-4 lg:grid-cols-[220px_1fr_220px]">
        <select value={optimize} onChange={(event) => setOptimize(event.target.value)} className="field-shell select-field rounded-[1.25rem] p-3.5">
          <option value="default">Default</option>
          <option value="thermal">Thermal</option>
          <option value="signal">Signal</option>
        </select>
        <label className="checkbox-row field-shell flex items-center gap-3 rounded-[1.25rem] px-4 py-3 text-sm text-muted">
